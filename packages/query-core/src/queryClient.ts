...
export class QueryClient {
  ...
  >(
    ...
    const parsedOptions = arg1
    const cachedData = this.getQueryData<TData>({
      queryKey: parsedOptions.queryKey,
    })
  ...
  setQueryData<TQueryFnData>(
    ...
    const query = this.queryCache.find<TQueryFnData>({
      queryKey,
    })
  ...
  cancelQueries(
    ...
    if (typeof options.revert === 'undefined') {
      options.revert = true
    }

    const promises = notifyManager.batch(() =>
      this.queryCache.findAll(filters).map((query) => query.cancel(options)),
    )
